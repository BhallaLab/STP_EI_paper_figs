...
noSTPData = "NO_STP/fig6supp_modelName_BothPresyn90_noSTP.g.h5"
yesNMDAData = "NO_STP/yesNMDAR_orig_0.h5"
SAMPLE_FREQ = 20000
chemDt = 0.0005
SAMPLE_TIME = 11
...
def panelE_epspVsTime( ax, column, pk5, pk15, perCell=None, free_ymax=False, header="" ):
    pk5 = np.array( pk5 )
    pk15 = np.array( pk15 )
    ...
    label = "A" + ["i", "ii", "iii", "iv"][column]
    ax.text( -0.20, 1.10, label, fontsize = 22, weight = "bold", transform=ax.transAxes )
    if header:
        ax.set_title( header, fontsize = 18, fontweight = "normal" )
    return mean5, mean15

def panelF_epspVsISI( ax, column, pk5, pk15, perCell=None, free_ymax=False ):
    pk5 = np.array( pk5 )
    pk15 = np.array( pk15 )
    mean5 = np.mean( pk5, axis = 0 )
    mean15 = np.mean( pk15, axis = 0 )
...
def printPanelStats( datasets, perCell_expt ):
    """Print Panel A and B statistics with per-cell experimental range.

    datasets      : list of (label, pk5, pk15); datasets[0] is experiment.
    perCell_expt  : perCell dict returned by scanData on the experimental data.
    """
    t2_mask      = PulseTrain / SAMPLE_FREQ > 2.0
    pt           = PulseTrain / SAMPLE_FREQ
    sim_datasets = datasets[1:]
    sim_labels   = [d[0] for d in sim_datasets]
    RLBL, RCOL, SCOL = 26, 20, 28

    def fmt( val, err ):
        return f"{val:8.3g} ±{err:7.3g}"

    def yn( val, lo, hi ):
        return "YES" if lo <= val <= hi else "NO "

    def hdr_line():
        return ( f"{'':>{RLBL}}"
                 + f"{'Expt range':>{RCOL}}"
                 + "".join( f"{lb:>{SCOL}}" for lb in sim_labels ) )

    def per_cell_t2( sq_key ):
        out = []
        for data in perCell_expt.values():
            cpk = data[f'pk{sq_key}']
            if len(cpk) < 2:
                continue
            arr = np.array(cpk)
            out.append( float(np.mean( np.mean(arr[:, t2_mask], axis=1) )) )
        return out

    def per_cell_slope( sq_key ):
        out = []
        for data in perCell_expt.values():
            cpk = data[f'pk{sq_key}']
            if len(cpk) < 2:
                continue
            lf = doLinFit( pt[:NUMPULSE], np.array(cpk)[:, :NUMPULSE], "", silent=True )
            if lf:
                out.append( lf['slope'] )
        return out

    def per_cell_tau( sq_key ):
        out = []
        for data in perCell_expt.values():
            cpk = data[f'pk{sq_key}']
            if len(cpk) < 2:
                continue
            ef = doExpFitISI( np.array(cpk), "", silent=True )
            if ef:
                out.append( ef['tau'] )
        return out

    def print_row( row_label, sq_key, mode, cell_func ):
        cell_vals = cell_func( sq_key )
        if not cell_vals:
            return
        lo, hi  = min(cell_vals), max(cell_vals)
        rng_str = f"[{lo:.3g}, {hi:.3g}]"
        row     = f"{row_label:<{RLBL}}{rng_str:>{RCOL}}"
        for _, pk5, pk15 in sim_datasets:
            pk  = pk5 if sq_key == "5" else pk15
            arr = np.array(pk)
            if mode == "t2":
                tm     = np.mean( arr[:, t2_mask], axis=1 )
                m, se  = float(np.mean(tm)), float(np.std(tm) / np.sqrt(len(tm)))
                cell   = fmt(m, se) + "  " + yn(m, lo, hi)
            elif mode == "slope":
                lf   = doLinFit( pt[:NUMPULSE], arr[:, :NUMPULSE], "", silent=True )
                cell = ( fmt(lf['slope'], lf['slope_err']) + "  " + yn(lf['slope'], lo, hi)
                         if lf else "N/A" )
            else:  # tau
                ef   = doExpFitISI( arr, "", silent=True )
                cell = ( fmt(ef['tau'], ef['tau_err']) + "  " + yn(ef['tau'], lo, hi)
                         if ef else "N/A" )
            row += f"{cell:>{SCOL}}"
        print( row )
    ...
    print( "\n=== Panel A: EPSP vs Time ===" )
    print( hdr )
    print( sep )
    print_row( "5sq   t>2s mean (mV)", "5",  "t2",    per_cell_t2    )
    print_row( "15sq  t>2s mean (mV)", "15", "t2",    per_cell_t2    )
    print_row( "5sq   slope (/s)",     "5",  "slope", per_cell_slope )
    print_row( "15sq  slope (/s)",     "15", "slope", per_cell_slope )

    print( "\n=== Panel B: EPSP vs ISI ===" )
    print( hdr )
    print( sep )
    print_row( "5sq   ISI tau (s)",  "5",  "tau", per_cell_tau )
    print_row( "15sq  ISI tau (s)", "15",  "tau", per_cell_tau )
    print()
...
def main():
    ...
    df_expt = pandas.read_hdf( args.fname )
    df_sim = pandas.read_hdf( args.fname2 )
    df_noSTP = pandas.read_hdf( noSTPData )
    df_yesNMDA = pandas.read_hdf( yesNMDAData )

    # Scan peaks from each dataset
    pk5_expt, pk15_expt, _, _, _, perCell_expt = scanData( df_expt, args )
    pk5_sim,     pk15_sim,     *_ = scanData( df_sim,     args )
    pk5_noSTP,   pk15_noSTP,   *_ = scanData( df_noSTP,   args )
    pk5_yesNMDA,pk15_yesNMDA,*_ = scanData( df_yesNMDA,args )

    HEADERS = ["Experiment", "Reference Sim", "No STP", "Active NMDAR"]
    datasets = [
        ( HEADERS[0], pk5_expt,    pk15_expt    ),
        ( HEADERS[1], pk5_sim,     pk15_sim     ),
        ( HEADERS[2], pk5_noSTP,   pk15_noSTP   ),
        ( HEADERS[3], pk5_yesNMDA, pk15_yesNMDA ),
    ]
    printPanelStats( datasets, perCell_expt )
    fig = plt.figure( figsize = (20, 8) )
    gs = fig.add_gridspec( 2, 4 )

    # Row A: EPSP vs Time (panel E)
    panelE_epspVsTime( fig.add_subplot(gs[0, 0]), 0, pk5_expt,    pk15_expt,    header=HEADERS[0] )
    panelE_epspVsTime( fig.add_subplot(gs[0, 1]), 1, pk5_sim,     pk15_sim,     header=HEADERS[1] )
    panelE_epspVsTime( fig.add_subplot(gs[0, 2]), 2, pk5_noSTP,   pk15_noSTP,   header=HEADERS[2], free_ymax=True )
    panelE_epspVsTime( fig.add_subplot(gs[0, 3]), 3, pk5_yesNMDA, pk15_yesNMDA, header=HEADERS[3] )

    # Row B: EPSP vs ISI (panel F)
    panelF_epspVsISI( fig.add_subplot(gs[1, 0]), 0, pk5_expt,    pk15_expt )
    panelF_epspVsISI( fig.add_subplot(gs[1, 1]), 1, pk5_sim,     pk15_sim )
    panelF_epspVsISI( fig.add_subplot(gs[1, 2]), 2, pk5_noSTP,   pk15_noSTP,   free_ymax=True )
    panelF_epspVsISI( fig.add_subplot(gs[1, 3]), 3, pk5_yesNMDA, pk15_yesNMDA )
