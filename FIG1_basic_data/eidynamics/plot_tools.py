...
flare   = mpl.colormaps["rocket"] # to assign bright color lumosity for higher values
crest   = mpl.colormaps["mako"] # to assign bright color lumosity for higher values
...
def add_floating_scalebar(ax, scalebar_origin=[0,0], xlength=1.0, ylength=1.0, labelx='', labely='', unitx='', unity='', fontsize=16, color='black', linewidth=2, pad=0.01, show_labels=False):
    ...
    if show_labels:    
        # write x axis label
        ax.text(x+xl/2, y-pad, labelx+' '+unitx, fontsize=fontsize, horizontalalignment='center', verticalalignment='top', color=color)
        # write y axis label
        ax.text(x-0.01, y+yl/2, labely+' '+unity, fontsize=fontsize, horizontalalignment='right', verticalalignment='center', rotation=90, color=color) # alignment of the rotated text as a block
...
def plot_data_from_df(df, data_start_column = 49, plot_mean=True, signals_to_plot=['Cell','FrameTTL', 'PD', 'Field'], signal_colors=['black','red','cyan','orange'], combine=False, fig=None, ax=None, signal_mapping={}, useFloatingScalebar = True):
    '''
    ...
    # if combine plots is true, draw all the 4 signals on a single plot
        ...
        for s,signal in enumerate(signals_to_plot):
            ...
            for i in range(sweeps):
                ...
                if i==0 and useFloatingScalebar:
                    if signal=='Cell':
                        scalebarx = 0.05*T
