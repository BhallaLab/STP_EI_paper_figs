...
def annotate_stat_stars(ax, pval, alpha=0.05, star_loc=[0.5,0.5], add_line=True, line_locs=[0,1,2,2], offset_btw_star_n_line=0.1, color='k', coord_system='axes', fontsize=14, add_n=False,**kwargs):
...
def pairwise_annotate_violin_plot(ax, df, x='', y='', stat=wilcoxon, add_line=False, offset=0.1, color='black', coord_system='axes', fontsize=14, add_n=False, annotate='both', **kwargs ):
    '''
    ...
    for i in unique_values:
        for j in unique_values:
            if i < j:
                ...
                xpos = (xipos + xjpos) / 2
                ypos = (0.9 + counter * 0.11) * original_ylim
                ...
                if annotate == 'both': # annotate both significant and non-significant pvals
                    ...
                    annotate_stat_stars(ax, pval, star_loc=[xpos, ypos], add_line=True, line_locs=[xipos, xjpos, ypos, ypos], offset_btw_star_n_line=offset, color=color, coord_system=coord_system, fontsize=14, zorder=10)
...
def pairwise_draw_and_annotate_line_plot(ax, df, x='', y='', hue='', draw=True, kind='violin', palette='viridis', split_violins=False, dodge=True, stat_across='hue', stat=kruskal, skip_first_xvalue=True, annotate_wrt_data=False, offset_btw_star_n_line=0.1, color='black', coord_system='data', fontsize=14, zorder=10, add_n=False):
    ...
    if stat_across == 'hue':
        ...
        for ix, x_val in enumerate(x_values):
            ...
            annotate_stat_stars(ax, kruskal_pval, star_loc=[xpos, ypos], add_line=False, color=color, coord_system=coord_system, fontsize=14, zorder=10)
            if add_n:
                # add annotation of size of the groups
        ...
        for ix, i in enumerate(unique_values):
            for jx, j in enumerate(unique_values):
                if ix > jx:
                    ...
                    annotate_stat_stars(ax, pval, star_loc=[xpos, ypos], add_line=True, line_locs=[xipos, xjpos, ypos, ypos], color=color, coord_system=coord_system, fontsize=14, zorder=10)
                    if add_n:
                        # add annotation of size of the groups
