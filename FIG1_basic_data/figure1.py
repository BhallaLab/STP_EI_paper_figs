...
def placeImg( ax, panel, file ):
    ax.text(-0.1, 0.95, panel, transform=ax.transAxes, size=20, weight='bold')
    image1_path = paper_figure_export_location / file
    im1 = Image.open(image1_path)
    imw, imh = im1.size
    #im1 = ax1a.imshow(im1, extent=[0, imw*1.1, 0, imh*1.1])
    im1 = ax.imshow(im1)
    ax.axis('off')
    #ax.set_anchor('NW')
    ...
    pos = ax.get_position()
    scale = 1.4
    #newLeft = pos.x0 - (pos.width*scale - pos.width)/4
    #newBottom = pos.y0 - (pos.height*scale - pos.height)*0.75
    if panel == 'A':
        newLeft = pos.x0 * 0.28
    else:
        newLeft = pos.x0 * 0.95
    newBottom = pos.y0 * 1.07
    ax.set_position([newLeft, newBottom, pos.width*scale, pos.height*scale])
...
def main():
    ...
    plt.close('all')
    fig1 = plt.figure(figsize=(15, 18), constrained_layout = True )
    gs = GridSpec(nrows=4, ncols=3, figure=fig1)
    ax1a, ax1b, ax1d, ax1e, ax1f, ax1g, ax1h, ax1i, ax1j, ax1k, ax1l, ax1m = [fig1.add_subplot(gs[ii//3, ii%3]) for ii in range( 12 ) ]

    # have more space between suplots
    fig1.subplots_adjust(hspace=0.5, wspace=0.5)

    # -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    # Fig 1A,B: Slice, polygon projection, and recording electrodes
    placeImg( ax1a, 'A', 'Fig1A.png' )
    placeImg( ax1b, 'B', 'Fig1B.png' )

    # # Fig 1C: Protocol Freq Sweep
    ax1d.text(-0.1, 1.1, 'C', transform=ax1d.transAxes, size=20, weight='bold')
    ax1d.text(0.05, 0.91, 'Trig', transform=ax1d.transAxes, size=14, color='red')
    ax1d.text(0.05, 0.71, 'Opto', transform=ax1d.transAxes, size=14, color='cyan')
    ax1d.text(0.05, 0.5, 'EPSP', transform=ax1d.transAxes, size=14, color='black')
    ax1d.text(0.05, 0.1, 'fEPSP', transform=ax1d.transAxes, size=14, color='orange')
    ...
    fig1, ax1d, signal_mapping = plot_tools.plot_data_from_df(data, data_start_column=49,combine=True, fig=fig1, ax=ax1d, useFloatingScalebar = False )
    ...
    ax1d.get_yaxis().set_visible(False)
    sns.despine(ax=ax1d, top=True, right=True, left=True, bottom=False, offset=10 )
    ...
    ax1e.text(-0.1, 1.1, 'D', transform=ax1e.transAxes, size=20, weight='bold')
    ...
    plot_tools.add_floating_scalebar(ax1e, scalebar_origin=[0.05,0.4], xlength=0.1, ylength=0.1, labelx='100', labely='', unitx='ms', unity='mV', fontsize=14, color='black', linewidth=2, pad=0.01, show_labels=True)

    # # -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    # # # Fig 1F: CA3 cell_fpr vs field_fpr
    ax1f.text(-0.1, 1.1, 'E', transform=ax1f.transAxes, size=20, weight='bold')
    ...
    ax1f_pvalues, ax1f_nvalues=stat_annotate.pairwise_annotate_violin_plot(
        ax1f, dftemp, x='numSq', y='field_fpr_p2p', stat=mannwhitneyu, 
        add_line=True, offset=0.18, color='black', add_n=False, 
        coord_system='', fontsize=14, zorder=10)

    # set labels
    ax1f.set_ylabel('First Peak fEPSP (mV)')
    ax1f.set_xlabel('Num Squares per Pattern')
    # legend off
    ax1f.legend([],[], frameon=False)
    # remove top and right spines
    #ax1f.spines['top'].set_visible(False)
    ...
    ax1g.text(-0.1, 1.1, 'F', transform=ax1g.transAxes, size=20, weight='bold')
    ...
    ax1g.text(0.45, 0.9, f"$r^2$ = {resultsCA3.rvalue**2:.2f}\n p << 0.001", transform=ax1g.transAxes, fontsize=14)
    ax1g.set_xlabel('First Peak EPSC (pA)')
    ax1g.set_ylabel('fEPSP (mV)')
    # despine
    ax1g.set_xlim([0, 65])
    ...
    ax1h.text(-0.1, 1.1, 'G', transform=ax1h.transAxes, size=20, weight='bold')
    ax1h.set_xlim([0, 0.5])
    ...
    plot_tools.add_floating_scalebar(ax1h, scalebar_origin=[0.05,0.5], xlength=0.1, ylength=2.0, labelx='100', labely='', unitx='ms', unity='mV', fontsize=14, color='black', linewidth=2, pad=0.2, show_labels=True)

    # # -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    # # # Fig 1I: First peak response | cell_fpr_max vs numSq
    ax1i.text(-0.1, 1.1, 'H', transform=ax1i.transAxes, size=20, weight='bold')
    ...
    ax1i.set_ylabel('First Peak CA1 EPSP (mV)')
    ax1i.set_xlabel('Num Squares per Pattern')
    ...
    ax1i_pvalues, ax1i_nvalues = stat_annotate.pairwise_annotate_violin_plot(ax1i, dftemp, x='numSq', y='PSC_0', stat=mannwhitneyu, add_line=True, 
            offset=0.2, color='black', add_n=False, coord_system='', 
    ...
    ax1j.text(-0.1, 1.1, 'I', transform=ax1j.transAxes, size=20, weight='bold')
    ...
    # # add r2 and p value to the plot as text annotation
    ax1j.text(0.45, 0.80, f"y  = {results.slope:.2f}x + {results.intercept:.2f},\n $r^2 = {results.rvalue**2:.2f}$,\n p << 0.001", transform=ax1j.transAxes, fontsize=14)
    ...
    # # set labels
    ax1j.set_ylabel('Normalized fEPSP (mV)')
    ...
    ax1k.text(-0.1, 1.1, 'J', transform=ax1k.transAxes, size=20, weight='bold')
    # ax1k.set_xlim([0, 0.5])
    ...
    ax1k, ax1j_insets = plot_tools.draw_pulse_response_snippets(dftemp, ax1k, patterns=range(55), palette=color_squares_EI, between='clampPotential', hue='numSq',stim_offset=1750, stim_scale=200, grid_size=11)
    # set ticks
    ax1k.set_xticks([])
    ax1k.set_yticks([])
    # axes spines to remove
    ...
    plot_tools.add_floating_scalebar(ax1k, scalebar_origin=[0.05,0.35], xlength=0.1, ylength=200, labelx='100', labely='', unitx='ms', unity='pA', fontsize=14, color='black', linewidth=2, pad=40, show_labels=True)

    # # ----------------------------------------------------------
    # # # Fig 1l: Trend in the field response across pulses
    ax1l.text(-0.1, 1.1, 'K', transform=ax1l.transAxes, size=20, weight='bold')
    ...
    for clamp in [-70,0]:
        ...
        stat_annotate.annotate_stat_stars(ax1l, pval, alpha=0.9, 
            star_loc=[2,1950], add_line=True, line_locs=[1,3,1900,1900],
            offset_btw_star_n_line=0.1, color='black', coord_system='data',)
        for s, violin in enumerate(parts['bodies']):
            sq = [1,5,15][s]
            color = color_squares_EI[clamp][sq]
            color = mpl.colors.to_rgba(color, alpha=1.0)
    ...
    ax1l.set_ylabel('First Peak CA1 EPSC (pA)')
    ax1l.set_xlabel('Num Squares per Pattern')
    #ax1l.legend(*zip(*labels), loc='lower left', bbox_to_anchor=(0,0), frameon=False, ncols=1, fontsize=14)
    # remove top and right spines
    ...
    ax1m.text(-0.1, 1.1, 'L', transform=ax1m.transAxes, size=20, weight='bold')
    ...
    sns.despine(ax=ax1m, left=False, right=True, top=True, bottom=False,
            offset=10, trim=True)
    ax1m.set_xlabel('Time (s)')
    ax1m.set_ylabel('Synaptic current (pA)')
    ...
    # # -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    for ax in [ax1a, ax1b, ax1d, ax1e, ax1f, ax1g, ax1h, ax1i, ax1j, ax1k, ax1l, ax1m]:
...
def extended_data_figure():
    ...
    stat_annotate.pairwise_annotate_violin_plot(ax1s2, df_temp, x='numSq', y='cell_fpr_max', stat=mannwhitneyu, add_line=True, offset=0.05, color='black', coord_system='data', fontsize=14, zorder=10)
