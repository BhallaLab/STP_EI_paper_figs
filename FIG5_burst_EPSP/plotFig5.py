...
def panelEPSPmax( ax, df, simEPSPmax, label ):
    ...
    # ── Simulated: same trend tests (4 points) ────────────────────────────────
    sim_slope, sim_intercept, sim_r_value, sim_p_value, sim_std_err = linregress(freqs, simEPSPmax)
    print("\n=== Simulated data ===")
    print("Linear regression results:")
    print(f"  Slope: {sim_slope:.5f}")
    print(f"  Intercept: {sim_intercept:.5f}")
    print(f"  R-squared: {sim_r_value**2:.4f}")
    print(f"  P-value: {sim_p_value:.4f}")

    sim_spearman_corr, sim_spearman_p_value = stats.spearmanr(freqs, simEPSPmax)
    print("\nSpearman's rank correlation results:")
    print(f"  Spearman correlation: {sim_spearman_corr:.4f}")
    print(f"  P-value: {sim_spearman_p_value:.4f}")

    sim_kendall_tau, sim_kendall_p_value = stats.kendalltau(freqs, simEPSPmax)
    print("\nKendall's tau correlation results:")
    print(f"  Kendall's tau: {sim_kendall_tau:.4f}")
    print(f"  P-value: {sim_kendall_p_value:.4f}")

    # ── Per-frequency: one-sample t-test + Wilcoxon (expt distribution vs sim value) ──
    print("\n=== Per-frequency: Expt distribution vs Sim value ===")
    for i, ff in enumerate(freqs):
        expt_vals = ne[:, i]
        sim_val = simEPSPmax[i]
        zscore = (sim_val - np.mean(expt_vals)) / np.std(expt_vals, ddof=1)
        t_stat, t_p = stats.ttest_1samp(expt_vals, sim_val)
        diffs = expt_vals - sim_val
        try:
            w_stat, w_p = stats.wilcoxon(diffs)
            w_str = f"W={w_stat:.1f}, p={w_p:.4f}"
        except ValueError as e:
            w_str = f"n/a ({e})"
        print(f"\n  {ff} Hz | expt mean={np.mean(expt_vals):.3f} ± {np.std(expt_vals, ddof=1):.3f},"
              f" sim={sim_val:.3f}, z={zscore:.2f}")
        print(f"    one-sample t-test : t={t_stat:.3f}, p={t_p:.4f}")
        print(f"    Wilcoxon signed-rank: {w_str}")
        print(f"Within range = {'YES' if (expt_vals.min() < sim_val) and (expt_vals.max()>sim_val) else 'NO' }")
        print(f"Range[e,e,sim] = [{expt_vals.min():.3f}, {expt_vals.max():.3f}, {sim_val:.3f}]")

    # ── Bootstrap 95% CI on experimental slope; check if sim slope falls inside ──
    rng = np.random.default_rng(42)
    n_cells = ne.shape[0]
    boot_slopes = np.array([
        linregress(ft, rng.choice(ne, n_cells, replace=True).flatten())[0]
        for _ in range(10000)
    ])
    ci_low, ci_high = np.percentile(boot_slopes, [2.5, 97.5])
    print("\n=== Bootstrap 95% CI on experimental regression slope (10 000 resamples) ===")
    print(f"  Expt slope CI : [{ci_low:.5f}, {ci_high:.5f}]")
    print(f"  Sim slope     : {sim_slope:.5f}")
    print(f"  Sim slope within CI: {ci_low <= sim_slope <= ci_high}")
...
def panelCond( ax, dcell, freq, label ):
    ...
    low = max( epsg*1e-3 )
    ax.plot( tepsg, pt * low/2.5 - low*1.2, "g", label = "Trig" )
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
...
def main():
    ...
    parser.add_argument( "-s", "--simFname", type = str, help = "Optional: File with simdata. Required to be hdf5.", default = "fig5burst_orig_0.h5" )
    parser.add_argument( "-c", "--cell", type = int, help = "Optional: Cell number to use. default = 3402", default = 3402 )
