...
def panelD_fepspVsTime( ax, fpk5, fpk15 ):
    ...
    ax.text( -0.20, 1.10, "D", fontsize = 22, weight = "bold", transform=ax.transAxes )
    ax.text( 0.20, 1.10, "fEPSP", fontsize = 16, transform=ax.transAxes )


def panelE_epspVsTime( ax, pk5, pk15 ):
    pk5 = np.array( pk5 )
    pk15 = np.array( pk15 )
    mean5 = np.mean( pk5, axis = 0 )
    mean15 = np.mean( pk15, axis = 0 )
    ...
    ax.text( 0.20, 1.10, "EPSP", fontsize = 16, transform=ax.transAxes )

def panelF_fepspProbVsTime( ax, fpk5, fpk15 ):
    all5 = np.array( [ ff for ff, ee in fpk5 ] )
    all15 = np.array( [ ff for ff, ee in fpk15 ] )
    #pk5 = np.array( pk5 )
    #pk15 = np.array( pk15 )
    ...
    ax.set_ylim( -5, 105 )
    ax.text( -0.20, 1.1, "F", fontsize = 22, weight="bold", transform=ax.transAxes )
    ax.text( 0.20, 1.10, "fEPSP", fontsize = 16, transform=ax.transAxes )

def panelG_epspProbVsTime( ax, pk5, pk15 ):
    pk5 = np.array( pk5 )
    pk15 = np.array( pk15 )
    pk5 = (pk5 > EPSPTHRESH)
    prob5 = np.sum( pk5, axis = 0 ) * 100 / len( pk5 )
...
def panelH_fepspVsISI( ax, fpk5, fpk15 ):
    all5 = np.array( [ ff for ff, ee in fpk5 ] )
    all15 = np.array( [ ff for ff, ee in fpk15 ] )
    mean5 = np.mean( all5, axis = 0 )
    mean15 = np.mean( all15, axis = 0 )
    ...
    ax.text( -0.20, 1.10, "H", fontsize = 22, weight = "bold", transform=ax.transAxes )
    ax.text( 0.20, 1.10, "fEPSP", fontsize = 16, transform=ax.transAxes )
...
def panelI_epspVsISI( ax, pk5, pk15 ):
    pk5 = np.array( pk5 )
    pk15 = np.array( pk15 )
    mean5 = np.mean( pk5, axis = 0 )
    mean15 = np.mean( pk15, axis = 0 )
...
def panelBC_epspVsField( ax, fpk, label ):
    color = "orange" if label == "C" else "blue"
    ...
    print( "Panel {}: slope = {:.3f}, intcpt={:.3f}, r={:.3f}, p={:.3f}".format( label, fit[0], fit[1], fit[2], fit[3] ) )
    ...
    ax.text( 0.10, 1.08, "r={:.2f}".format(fit[2]), fontsize = 16, transform=ax.transAxes )
    ax.text( 0.10, 0.88, "m={:.2f}".format(fit[0]), fontsize = 16, transform=ax.transAxes )
...
def panelA_SampleTrace( ax, dcell ):
    ...
    '''
    df = dcell.loc[(ipat == 47)]
    print( df['sweep'].unique() )
    quit()
    '''
    #df = dcell.loc[(ipat == 46) & (dcell['sweep'] == 12)]
    df = dcell.loc[(ipat == 47) & (dcell['sweep'] == 11)]
    ...
    ax.plot( tepsp, epsp[:len(tepsp)], "b", label = "EPSP   " )
    ...
    #ax.plot( [0,0,0.25], [7.5,5,5], color="black", linewidth=2.5 )
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ...
    ax.set_ylabel( "EPSP (mV); 10xfEPSP (mV)" )
    ax.set_xlabel( "Time (s)" )
...
def panelLM_varianceHisto( ax1, ax2, patDict ):
    ...
    ax1.text( 0.20, 1.10, "fEPSP", fontsize = 16, transform=ax1.transAxes )
    ax2.spines['top'].set_visible(False)
    ax2.spines['right'].set_visible(False)
    ax2.set_xlabel( "std:EPSP" )
    ax2.set_ylabel( "#" )
    ax2.text( -0.20, 1.05, "M", fontsize = 22, weight = "bold", transform=ax2.transAxes )
    ax2.text( 0.20, 1.10, "EPSP", fontsize = 16, transform=ax2.transAxes )
...
def main():
    ...
    panelBC_epspVsField( fig.add_subplot(gs[2,0]), fpk5, "B" )
    panelBC_epspVsField( fig.add_subplot(gs[2,1]), fpk15, "C" )
    panelD_fepspVsTime( fig.add_subplot(gs[3,0]), fpk5, fpk15 )
    panelE_epspVsTime( fig.add_subplot(gs[3,1]), pk5, pk15 )
    panelF_fepspProbVsTime( fig.add_subplot(gs[4,0]), fpk5, fpk15 )
    panelG_epspProbVsTime( fig.add_subplot(gs[4,1]), pk5, pk15 )
    panelH_fepspVsISI(  fig.add_subplot(gs[5,0]), fpk5, fpk15 )
    panelI_epspVsISI(  fig.add_subplot(gs[5,1]), pk5, pk15 )
    panelJ_fpkHisto( fig.add_subplot(gs[6,0]), fpk5, fpk15 )
    panelK_epkHisto( fig.add_subplot(gs[6,1]), pk5, pk15 )
    panelLM_varianceHisto( fig.add_subplot(gs[7,0]), fig.add_subplot(gs[7,1]), patDict )
