...
def load( scaleParam = [], chemFile = None ):
    ...
    tau2 = 0.04  # Replace tau with a cell-specific value obtained from Look up fitEphysToTau.py
    RM = '1.0'
    RA = '1.0'
    ...
    rdes = rd.rdesigneur(
        ...
        adaptorList = [ 
            # Note that 'n' here is number of vesicles, not of transmitter.
            [ 'glu/glu', 'n', 'gluR', 'activation', 0.0, 0.1e-9 ],
            [ 'GABA/GABA', 'n', 'GABAR', 'activation', 0.0, 50 ],
        ],
    )
    gabaReceptor = moose.element( '/library/GABAR' )
    gabaReceptor.tau2 = tau2
    gabaReceptor.Ek = -0.07
    return rdes
