...
def doStats( df, alphaTab, alphaDelay, pkDelay ):
    print( float( df['intensity'] ) )

    if float( df['intensity'] ) > 90:
        #print( "STRONG RESPONSE FOR CELL ", df['cellID'] )
        return [], [], 0
        ...
        for pp in patList:
            ...
            for ss in sweepList:
                ...
                for seq in seqList:
                    ...
                    if float( dsweep.loc[dsweep['exptSeq'] == seq]['intensity'] ) < 90:
                        print( "Lo intensity, totNumPulse = ", totNumPulse )
                    if totNumPulse == 0:
                        continue
