...
stimList = []
pulseTrig = []
patternData = "../../../2022/VC_DATA/all_cells_surprise_CC_long.h5"
SAMPLE_FREQ = 20000
chemDt = 0.0005
SAMPLE_TIME = 5
...
def findPeaks( pkDelay, Vm, PulseTrain, width = 0.004 ):
...
def findMaxima( pkDelay, Vm, width = 0.002 ):
    widthSamples = int( np.round( width * SAMPLE_FREQ ) )
    half = widthSamples // 2
    lastPP = 0.0
    pks = []
    for idx in PulseTrain:
        idx2 = idx + pkDelay - widthSamples
        pks.append(max( Vm[idx2:idx2 + widthSamples*2] ))
...
def parseRow( df, cell ):
    ...
    if cell == 111:
        ...
        #print( "FIELD: ", np.mean( field ), min( field ), max( field ) )
        fpks = findMaxima( 100, field, width = 0.002 )
    else:
...
def panelA_SampleTrace( ax, dcell ):
    ...
    PulseTrain = np.arange( 0, NUM_SAMPLES, 1, dtype = int )[edges[:NUM_SAMPLES]]
    print( PulseTrain )

    baseline = min( np.percentile(epsp, 25 ), 0.0 )
    epsp -= baseline # hack to handle traces with large ipsps.
    tepsp = np.linspace( 0, SAMPLE_TIME, len(epsp) )
    pks = findPeaks( pkDelay, epsp, PulseTrain )
